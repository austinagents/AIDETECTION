...
export default async function HistoryPage() {
  ...
  return (
    <Shell>
      ...
      <p className="mt-2 text-sm text-slate-400">Past writing reviews and risk scores.</p>
      <Card className="mt-8 overflow-hidden">
        ...
        {analyses.map((item) => (
          <Link key={item.id} href={`/analysis/${item.id}`} className="grid grid-cols-[1fr_140px_120px_120px] items-center border-b border-ink-700 px-5 py-4 last:border-b-0 hover:bg-ink-800/60">
            ...
            <span className="flex items-center gap-3"><b>{item.overallRisk}</b><RiskBadge label={item.riskLabel} /></span>
          </Link>
        ))}
